...
On a une superglobale $_SESSION (on en fait un tableau)
   ...
   On va gérer cette variable de la manière suivante :
    ...
    - si un utilisateur est connecté $_SESSION["id"] contient l'id de l'utilisateur connecté
...
function session_activation() {
    ...
    if (session_isconnected()) {
        ...
        $utilisateurConnecte = new user(session_idconnected());
...
function session_isconnected() {
    ...
    // Paramètres : néant
    // Retour : true si on est connecte, false sinon
 ...
 function session_connect($id) {
    ...
    global $utilisateurConnecte;
    $utilisateurConnecte = new user($id);
 }
